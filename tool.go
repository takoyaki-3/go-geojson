...
func (geom *Geometry)Distance()float64{
	coordinates := geom.Coordinates.([][]float64)
	dis := 0.0
	for i:=0;i<len(coordinates)-1;i++{
		from := coordinates[i]
		to := coordinates[i+1]
		dis += hubenyDistance(point{Lat: from[1],Lon: from[0]},point{Lat: to[1],Lon: to[0]})
	}
	return dis
}

// 以下、https://qiita.com/tmnck/items/30b42ba5df28c38b0f89 様の記事を一部改変の上，利用しております。
func hubenyDistance(src, dst point) float64 {
